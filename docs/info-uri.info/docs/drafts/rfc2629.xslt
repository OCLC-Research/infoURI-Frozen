...
<xsl:template name="get-generator">
  <xsl:variable name="gen">
    ...
    <xsl:if test="contains('$Revision: 1.1 $',':')">
      <xsl:value-of select="concat('Revision ',normalize-space(translate(substring-after('$Revision: 1.1 $', 'Revision: '),'$','')),', ')" />
    </xsl:if>
    <xsl:if test="contains('$Date: 2004/07/12 18:53:51 $',':')">
      <xsl:value-of select="concat(normalize-space(translate(substring-after('$Date: 2004/07/12 18:53:51 $', 'Date: '),'$','')),', ')" />
    </xsl:if>
